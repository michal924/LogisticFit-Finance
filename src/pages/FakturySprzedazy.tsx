...
import { FileUp, Upload, Loader2, X, Check, Search, ChevronRight, RefreshCw } from 'lucide-react';
...
export default function FakturySprzedazy() {
  ...
  async function handleInfaktSync(force = false) {
    ...
    setSyncMsg(isBetterfly
      ? 'Pobieram faktury sprzedaży z Comarch Betterfly…'
      : (force ? 'Pełna archiwizacja — pobieram wszystkie dokumenty i załączniki…' : 'Pobieram i archiwizuję faktury sprzedaży z inFakt…'));
    try {
      const r = isBetterfly
        ? await syncFromBetterfly('sales', context, invoices, (done, total) => {
            setSyncMsg(`Synchronizuję… ${done}/${total}`);
          })
        : await syncFromInfakt('sales', context, invoices, (done, total, arch) => {
  ...
  useEffect(() => {
    ...
    const key = `autosync-sales-${context}`;
    ...
    (async () => {
      try {
        const n = isBetterfly
          ? await autoSyncDataBetterfly('sales', context, invoices)
          : await autoSyncData('sales', context, invoices);
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--lf-navy-900)', margin: 0 }}>Faktury sprzedaży</h1>
        ...
        <div style={{ background: '#fff', borderRadius: 10, border: '1px solid var(--border)', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} style={{ padding: 48, textAlign: 'center', color: 'var(--fg-3)' }}><Loader2 size={20} style={{ display: 'inline' }} /></td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={8} style={{ padding: 48, textAlign: 'center', color: 'var(--fg-3)' }}>Brak faktur</td></tr>
              ) : filtered.map(inv => {
                ...
                return (
                  ...
                  <tr onClick={() => setSelected(isActive ? null : inv)}
                    style={{ borderBottom: isActive ? 'none' : '1px solid var(--border)', cursor: 'pointer', background: isActive ? '#f4f6fb' : '#fff', transition: 'background .1s' }}
      ...
      {/* Modal */}
      {showForm && <InvoiceModal invoice={editing} type="sales" onSave={handleSave} onClose={() => { setShowForm(false); setEditing(null); }} />}
    </div>
  );
}
...
function InvoiceModal({ invoice, type, onSave, onClose }: { invoice: Invoice | null; type: 'sales' | 'cost'; onSave: (inv: Invoice) => Promise<void>; onClose: () => void; }) {
  ...
  const field = (label: string, key: keyof Invoice, t = 'text') => (
    <div>
      <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--fg-3)', display: 'block', marginBottom: 4 }}>{label}</label>
      <input type={t} value={(form[key] as string) || ''} onChange={e => setForm(f => ({ ...f, [key]: t === 'number' ? parseFloat(e.target.value) || 0 : e.target.value }))}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: 12, padding: 24, width: 520, maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{form.spId ? 'Edytuj fakturę' : 'Nowa faktura sprzedaży'}</h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer' }}><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            {field('Nr faktury', 'number')}
            {field('Kontrahent', 'counterparty')}
            {field('NIP', 'nip')}
            {field('Waluta', 'currency')}
            {field('Data wystawienia', 'issueDate', 'date')}
            {field('Termin płatności', 'dueDate', 'date')}
            {field('Netto (PLN)', 'netTotal', 'number')}
            {field('VAT (PLN)', 'vatTotal', 'number')}
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button type="submit" disabled={saving} style={{ padding: '8px 16px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
