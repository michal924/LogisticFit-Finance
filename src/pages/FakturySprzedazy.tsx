...
import { FileUp, Upload, Loader2, X, Check, Clock, Search, ChevronRight, RefreshCw } from 'lucide-react';
...
export default function FakturySprzedazy() {
  ...
  async function handleInfaktSync(force = false) {
    setSyncing(true);
    setSyncMsg(force ? 'Pełna archiwizacja — pobieram wszystkie dokumenty i załączniki…' : 'Pobieram i archiwizuję faktury sprzedaży z inFakt…');
    try {
      const r = await syncFromInfakt('sales', context, invoices, (done, total, arch) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const n = await autoSyncData('sales', context, invoices);
              ...
              {loading ? (
                ...
                return (
                  <tr key={inv.spId} onClick={() => setSelected(inv)}
                    style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer', background: isActive ? '#f4f6fb' : '#fff', transition: 'background .1s' }}
          ...
          {(selected.attachments?.length || selected.fileUrl) && (
            <div style={{ marginBottom: 12 }}>
              ...
              {(selected.attachments?.length
                ...
                : [{ name: 'Faktura (oryginał)', url: selected.fileUrl! }]
              ).map((doc, i) => (
